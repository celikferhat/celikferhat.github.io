...
jQuery(document).ready(function(){
	...
	arlo_tm_animate_bg_menu();
	arlo_tm_justified();
	arlo_tm_animate_about_new();
...
function arlo_tm_animate_about_new(){
	"use strict";
	jQuery(window).on('scroll',function(){
		var WinScroll		= jQuery(window).scrollTop();
		var menu			= jQuery('.arlo_tm_menu_animate');
...
function arlo_tm_animate_bg_menu(){
	"use strict";
	jQuery(window).on('scroll',function(){
		var WinScroll		= jQuery(window).scrollTop();
		var about			= jQuery('.arlo_tm_about_animate');
...
function arlo_tm_hero_heights(){
	...
	var menuHeight	= jQuery('.arlo_tm_univ_menu').outerHeight();
	var hero1		= jQuery('.arlo_tm_univ_hero_wrap');
...
function arlo_tm_anchor(){
	...
	jQuery('.arlo_tm_univ_menu .menu ul li a,.arlo_tm_topbar.new .dropdown .dropdown_inner ul li a,.arlo_tm_menu_animate .menu_inner .menu ul li a').off().on('click',function(e){
		e.stopPropagation();
		var element = jQuery(this);
		var url			= element.attr('href');
		if(url !== '#' && url.charAt(0) === '#'){
...
function arlo_tm_down(){
	...
	jQuery('.arlo_tm_arrow_new a').on('click',function(){
		if($.attr(this, 'href') !== '#'){
			$('html, body').animate({
...
function arlo_tm_animate_text(){
	...
	var animateSpan1			= jQuery('.arlo_tm_animation_text_word');
		...
		animateSpan1.typed({
			strings: ["Freelancer", "Web Developer", "Mobile Developer"],
			loop: true,
			startDelay: 1e3,
			backDelay: 2e3
		});
	...
	var animateSpan3			= jQuery('.arlo_tm_animation_text_word3');
...
function arlo_tm_portfolio_filter(){
	...
	if(jQuery().isotope) {
		...
		var filter		 = jQuery('.arlo_tm_portfolio_filter_second');
...
function arlo_tm_partners(){
	...
	"use strict";
		...
		var carousel6			= jQuery('.arlo_tm_testimonial_new .owl-carousel');
...
function arlo_tm_kenburn_slider(){
	...
	"use strict";
		...
		jQuery(function() {
			jQuery('.arlo_tm_kenburn_hero .overlay_slider,.arlo_tm_univ_hero_wrap .overlay_slider').vegas({
			timer:false,	
